...
dbFetchOrCreate <- function(db, name, f) {
  if (dbExists(db, name)) {
...
ngramsFH <- function(dt, name, n) {
  N <- dim(dt)[1]
  name1 <- paste0(name, "1grams")
  dt1 <- dbFetchOrCreate(dbCache, name1, function() dt)
  dt.acc <- dt1
  if (n>=2) {
    for (k in 2:n) {
      name.new <- paste0(name, k, "grams")
      dt.acc <- dbFetchOrCreate(dbCache, name.new, function() newKGramsTable(dt.acc, dt1, N, k))
    }
  }
}
